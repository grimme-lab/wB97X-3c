...
! read file ~/.basis and convert to ORCA style
call rdbas
write(7,'(''%basis'')')
do jat=1,94
...
! read file ~/.ecp and convert to ORCA style
call rdecp
do jat=1,94
...
!          no space in first column allowed in ecp file
      ...
      if(index(atmp,ang2(lmax(iat)+1)).eq.0) then
         ...
         &,ec2(3,l,npr(l,iat),iat),ec2(1,l,npr(l,iat),iat)
      ENDIF
      GOTO 12
   ENDIF
